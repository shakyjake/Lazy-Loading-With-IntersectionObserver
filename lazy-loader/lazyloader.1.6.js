/*
	...
	What's new in this version?
	 - Can add images to observation queue after-the-fact
*/
...
LazyLoader = function(Selector, ObserveImmediately, RootMargin){
	...
	_.RootMargin = typeof(RootMargin) === 'number' ? RootMargin : 100;
	...
	_.LoadResource = function(Holder){
		...
		AlsoLoad = document.querySelectorAll('[data-src="' + Source +'"]');/* Find images with the same source and load them too (binding cloned nodes is... uh... "unpredictable"... so this helps mitigate that) */
		if(!!AlsoLoad.length){
			if(_.UseObserver){
	...
	_.ObserveRetro = function(Element){
		_.Elements = document.querySelectorAll(_.Selector);/* HTMLCollection is readonly so we need to grab every single element again */
		if(_.UseObserver){
